...
const TestingRoom = () => {
  ...
  const generateRandomExam = async (duration) => {
    try {
      const response = await axios.post("http://localhost:8080/api/v1/exams/random", { duration });
  ...
  const handleStartExamClick = async () => {
    ...
    await generateRandomExam(duration);
